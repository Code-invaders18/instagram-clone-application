...
const Home =()=>{
    ...
    useEffect(()=>{
       Promise.all([
        ...
        fetch("http://localhost:8000/api/allquotes",{
            headers:{
                "Authorization":"Bearer "+localStorage.getItem("jwt")
            }
        }).then(res=>res.json())
        .then(quoteString=>{
            //  console.log(quoteString)
             setQuotedata(quoteString.quotes)
        })
        ...
        <div>
            {
             ...
             {quotedata.length === 0 && quotedata.map(index=>{console.log(index)})}
        <div>
            {
                quotedata && quotedata.map(quoteItem=>{
                    {/* console.log(quoteItem) */}
                    return(
                        <div className="site-card-border-less-wrapper">
                            <Card title={quoteItem.postedBy.name} bordered={false} style={{ width: 300 }}>
                            <div dangerouslySetInnerHTML={{__html:`${quoteItem.quote}`}}></div>
                            </Card>
                        </div>
                    )
                })
